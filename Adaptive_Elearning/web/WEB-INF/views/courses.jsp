                ...
                <aside class="col-lg-3">
                    <div class="glass sidebar-section p-4 mb-4">
                        ...
                        <div class="list-group list-group-flush">
                            <a href="?category=development" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center ${category == 'development' ? 'active' : ''}">
                                <span><i class="fas fa-code me-2 category-icon-dev"></i>Development</span>
                                <span class="badge bg-primary rounded-pill">${categoryCounts['Development'] != null ? categoryCounts['Development'] : 0}</span>
                            </a>
                            <a href="?category=marketing" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center ${category == 'marketing' ? 'active' : ''}">
                                <span><i class="fas fa-bullhorn me-2 category-icon-marketing"></i>Marketing</span>
                                <span class="badge bg-primary rounded-pill">${categoryCounts['Marketing'] != null ? categoryCounts['Marketing'] : 0}</span>
                            </a>
                            <a href="?category=business" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center ${category == 'business' ? 'active' : ''}">
                                <span><i class="fas fa-briefcase me-2 category-icon-business"></i>Business</span>
                                <span class="badge bg-primary rounded-pill">${categoryCounts['Business'] != null ? categoryCounts['Business'] : 0}</span>
                            </a>
                            <a href="?category=lifestyle" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center ${category == 'lifestyle' ? 'active' : ''}">
                                <span><i class="fas fa-heart me-2 category-icon-lifestyle"></i>Lifestyle</span>
                                <span class="badge bg-primary rounded-pill">${categoryCounts['Lifestyle'] != null ? categoryCounts['Lifestyle'] : 0}</span>
                            </a>
                            <a href="?category=personal" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center ${category == 'personal' ? 'active' : ''}">
                                <span><i class="fas fa-user-astronaut me-2 category-icon-personal"></i>Personal Development</span>
                                <span class="badge bg-primary rounded-pill">${categoryCounts['Personal Development'] != null ? categoryCounts['Personal Development'] : 0}</span>
                            </a>
                        </div>
                    </div>
                    <div class="glass sidebar-section p-4 mb-4">
                        ...
                        <div class="list-group list-group-flush">
                            <a href="?sort=newest${searchTerm != null ? '&search='+=searchTerm : ''}${not empty category ? '&category='+=category : ''}" class="list-group-item list-group-item-action ${sortBy == null || sortBy == 'newest' ? 'active' : ''}"><i class="fas fa-clock me-2"></i>Mới nhất</a>
                            <a href="?sort=popular${searchTerm != null ? '&search='+=searchTerm : ''}${not empty category ? '&category='+=category : ''}" class="list-group-item list-group-item-action ${sortBy == 'popular' ? 'active' : ''}"><i class="fas fa-fire me-2"></i>Phổ biến</a>
                            <a href="?sort=rating${searchTerm != null ? '&search='+=searchTerm : ''}${not empty category ? '&category='+=category : ''}" class="list-group-item list-group-item-action ${sortBy == 'rating' ? 'active' : ''}"><i class="fas fa-star me-2"></i>Đánh giá cao</a>
                            <a href="?sort=price${searchTerm != null ? '&search='+=searchTerm : ''}${not empty category ? '&category='+=category : ''}" class="list-group-item list-group-item-action ${sortBy == 'price' ? 'active' : ''}"><i class="fas fa-tag me-2"></i>Giá</a>
                        </div>
                    </div>
                    ...
                    <nav aria-label="Course pagination" class="mt-4">
                        <ul class="pagination justify-content-center">
                            <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
                                <a class="page-link" href="?page=${currentPage - 1}${not empty searchTerm ? '&search='+=searchTerm : ''}${not empty sortBy ? '&sort='+=sortBy : ''}${not empty category ? '&category='+=category : ''}" tabindex="-1"><i class="fas fa-chevron-left"></i></a>
                            </li>
                            <c:forEach begin="1" end="${totalPages}" var="i">
                                <li class="page-item ${currentPage == i ? 'active' : ''}">
                                    <a class="page-link" href="?page=${i}${not empty searchTerm ? '&search='+=searchTerm : ''}${not empty sortBy ? '&sort='+=sortBy : ''}${not empty category ? '&category='+=category : ''}">${i}</a>
                                </li>
                            </c:forEach>
                            <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
                                <a class="page-link" href="?page=${currentPage + 1}${not empty searchTerm ? '&search='+=searchTerm : ''}${not empty sortBy ? '&sort='+=sortBy : ''}${not empty category ? '&category='+=category : ''}"><i class="fas fa-chevron-right"></i></a>
                            </li>
                        </ul>
                    </nav>
                </main>
