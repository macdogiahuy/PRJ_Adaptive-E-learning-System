...
<%
    ...
    if (currentUser != null) {
        ...
        currentRole = currentUser.getRole() != null ? currentUser.getRole().toString() : "Admin";
    }
...
<body class="universe-theme">
    ...
    <div class="header universe-header">
        ...
        <div class="user-info">
            ...
            <small style="display: block; font-size: 12px; opacity: 0.7;">(<%= currentRole %>)</small>
            <% if (currentUser != null) { %>
                <small style="display: block; font-size: 10px; opacity: 0.6;">
                    ID: <%= currentUser.getId() != null ? currentUser.getId().toString().substring(0, 8) + "..." : "N/A" %>
                </small>
            <% } %>
        </div>
    </div>
    ...
    <script>
        ...
        <% if (currentUser != null) { %>
        console.log("Full Name: <%= currentUser.getFullName() %>");
        console.log("Email: <%= currentUser.getEmail() %>");
        console.log("User ID: <%= currentUser.getId() %>");
        <% } %>
    </script>
    
    <!-- Debug Panel (remove in production) -->
    <div style="position: fixed; bottom: 10px; right: 10px; background: rgba(0,0,0,0.8); color: white; padding: 10px; border-radius: 5px; font-size: 12px; max-width: 300px; display: none;" id="debugPanel">
        <strong>Session Debug Info:</strong><br>
        Username: <%= currentUsername %><br>
        Role: <%= currentRole %><br>
        <% if (currentUser != null) { %>
        Full Name: <%= currentUser.getFullName() %><br>
        Email: <%= currentUser.getEmail() %><br>
        User ID: <%= currentUser.getId() %><br>
        <% } else { %>
        No user session found!<br>
        <% } %>
        <button onclick="document.getElementById('debugPanel').style.display='none'" style="margin-top: 5px;">Close</button>
    </div>
    
    <!-- Toggle debug panel -->
    <button onclick="document.getElementById('debugPanel').style.display='block'" 
            style="position: fixed; bottom: 10px; right: 10px; background: #667eea; color: white; border: none; padding: 5px 10px; border-radius: 3px; font-size: 11px;">
        Debug
    </button>
</body>
</html>