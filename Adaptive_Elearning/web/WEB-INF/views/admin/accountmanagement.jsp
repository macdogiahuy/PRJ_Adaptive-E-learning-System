...
<%
    ...
    try {
        ...
        if (conn != null) {
            ...
            // Count total active users and get role statistics (excluding banned users)
            ...
            // Get role statistics (including inactive for admin display)
...
<head>
    ...
    <style>
        * {
        ...
        /* Custom Scrollbar Styles - Enhanced for better visibility */
        ...
        .table-wrapper::-webkit-scrollbar-thumb {
            background: linear-gradient(145deg, rgba(139, 92, 246, 0.8), rgba(6, 182, 212, 0.8));
            border-radius: 10px;
        ...
        .table-wrapper::-webkit-scrollbar-thumb:hover {
            background: linear-gradient(145deg, rgba(139, 92, 246, 1), rgba(6, 182, 212, 1));
        ...
        /* For Firefox - Enhanced vertical scroll */
        .table-wrapper {
            scrollbar-width: thin;
            scrollbar-color: rgba(139, 92, 246, 0.8) rgba(30, 27, 75, 0.8);
        }
        ...
        * {
        ...
        @media (max-width: 768px) {
            ...
            /* Enhanced mobile table scroll with better visibility */
            .table-wrapper::-webkit-scrollbar {
                width: 8px;
            }

            .table-wrapper::-webkit-scrollbar-thumb {
                background: linear-gradient(45deg, rgba(139, 92, 246, 0.9), rgba(6, 182, 212, 0.9));
                border-radius: 6px;
...
<body class="universe-theme">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize enhanced scrolling for table
            ...
            // Highlight user count if just banned/unbanned
            highlightUserCountIfChanged();
        ...
        // Enhanced scrolling functionality for table
        function initEnhancedScrolling() {
            ...
            scrollIndicator.style.cssText = `
                ...
                pointer-events: none;
            `;
            tableWrapper.parentElement.style.position = 'relative';
            tableWrapper.parentElement.appendChild(scrollIndicator);
            ...
            function updateScrollIndicator() {
                ...
                if (scrollHeight > clientHeight) {
                    ...
                    scrollIndicator.style.opacity = scrollTop > 10 ? '1' : '0';
                }
            }
            
            // Smooth scroll to top/bottom with keyboard shortcuts
            ...
            scrollDownBtn.style.cssText = scrollUpBtn.style.cssText;
            ...
            [scrollUpBtn, scrollDownBtn].forEach(btn => {
                btn.addEventListener('mouseenter', () => {
                    ...
                    btn.style.boxShadow = '0 8px 20px rgba(139, 92, 246, 0.5)';
                });
            ...
            scrollUpBtn.addEventListener('click', () => {
                tableWrapper.scrollBy({ top: -200, behavior: 'smooth' });
            });
            ...
            scrollButtons.appendChild(scrollUpBtn);
            scrollButtons.appendChild(scrollDownBtn);
            tableWrapper.parentElement.appendChild(scrollButtons);
            
            // Show/hide scroll buttons based on scroll position
            tableWrapper.addEventListener('scroll', function() {
                const { scrollTop, scrollHeight, clientHeight } = this;
                scrollUpBtn.style.display = scrollTop > 50 ? 'block' : 'none';
                scrollDownBtn.style.display = scrollTop < scrollHeight - clientHeight - 50 ? 'block' : 'none';
            });
            
            // Initial check
            const { scrollHeight, clientHeight } = tableWrapper;
            if (scrollHeight <= clientHeight) {
        ...
        function deactivateUser(userId) {
            if (confirm('Bạn có chắc chắn muốn vô hiệu hóa người dùng này? Hành động này sẽ chuyển trạng thái của họ thành "Inactive".')) {
                ...
                console.log('Banning user:', userId, '- Decreasing count by 1');
                updateUserCount(-1);
        ...
        function unbanUser(userId) {
            if (confirm('Bạn có chắc chắn muốn khôi phục người dùng này? Họ sẽ được chuyển về vai trò Learner.')) {
                ...
                // Update counter immediately for better UX
                console.log('Unbanning user:', userId, '- Increasing count by 1');
                updateUserCount(+1);
        ...
        // Function to highlight user count after ban/unban action
        function highlightUserCountIfChanged() {
            const urlParams = new URLSearchParams(window.location.search);
            const deactivated = urlParams.get('deactivated');
            const unbanned = urlParams.get('unbanned');
            
            if (deactivated === 'success' || unbanned === 'success') {
                const totalUsersElement = document.getElementById('totalUsersCount');
                const statCard = totalUsersElement ? totalUsersElement.closest('.stat-card') : null;
                
                if (statCard) {
                    // Add highlight animation
                    statCard.style.transition = 'all 0.5s ease';
                    statCard.style.transform = 'scale(1.05)';
                    statCard.style.boxShadow = '0 20px 40px rgba(139, 92, 246, 0.6), 0 0 30px rgba(6, 182, 212, 0.4)';
                    
                    // Flash the number
                    const actionType = deactivated === 'success' ? 'ban' : 'unban';
                    const color = actionType === 'ban' ? '#EF4444' : '#10B981';
                    
                    totalUsersElement.style.transition = 'all 0.5s ease';
                    totalUsersElement.style.color = color;
                    totalUsersElement.style.transform = 'scale(1.2)';
                    totalUsersElement.style.textShadow = `0 0 20px ${color}`;
                    
                    // Create celebration particles
                    createCelebrationParticles(statCard, actionType);
                    ...
                    setTimeout(() => {
                        ...
                        totalUsersElement.style.transform = '';
                        totalUsersElement.style.textShadow = '';
                    }, 1500);
                }
            }
        }
        ...
        function createCelebrationParticles(element, actionType) {
            const rect = element.getBoundingClientRect();
            const particles = actionType === 'ban' ? 12 : 16;
            const colors = actionType === 'ban' 
        ...
        // Function to update user count in stat card
        function updateUserCount(change) {
            ...
            if (totalUsersElement) {
                ...
                totalUsersElement.style.color = change > 0 ? '#10B981' : '#EF4444'; // Green for increase, red for decrease
                ...
                // Add sparkle effect for visual feedback
                createSparkleEffect(totalUsersElement.closest('.stat-card'));
            }
        }

        // Function to create sparkle effect on stat card
        function createSparkleEffect(element) {
            ...
            for (let i = 0; i < sparkles; i++) {
                ...
                sparkle.animate([
                    ...
                    { 
                        transform: `scale(0) rotate(${Math.random() * 720}deg)`, 
                        opacity: 0 
                    }
                ], {
