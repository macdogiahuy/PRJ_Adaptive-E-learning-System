...
public class CourseManagementController {
    ...
    public List<Course> getCourses(String searchTerm, String sortBy, String category, int limit, int offset)
            throws SQLException {
        ...
        sql.append("LEFT JOIN Categories cat ON c.LeafCategoryId = cat.Id ");
        ...
        if (category != null && !category.trim().isEmpty()) {
            ...
            sql.append("LOWER(cat.Name) = LOWER(?) ");
        ...
        if (sortBy != null && !sortBy.trim().isEmpty()) {
            switch (sortBy.toLowerCase()) {
                case "newest":
                    ...
                    break;
                case "popular":
                    sql.append("ORDER BY c.LearnerCount DESC ");
                    break;
                case "rating":
                    sql.append(
                            "ORDER BY c.TotalRating / CASE WHEN c.RatingCount = 0 THEN 1 ELSE c.RatingCount END DESC ");
                    break;
                case "price":
                    sql.append("ORDER BY CASE WHEN c.Discount > 0 AND c.DiscountExpiry > GETDATE() " +
                            "THEN c.Price * (1 - c.Discount) ELSE c.Price END ");
                    break;
                default:
                    sql.append("ORDER BY c.CreationTime DESC ");
            }
    ...
    public int getTotalCourseCount(String searchTerm, String category) throws SQLException {
        StringBuilder sql = new StringBuilder("SELECT COUNT(*) FROM Courses c ");
    ...
    public int getTotalCourseCount(String searchTerm) throws SQLException {
        return getTotalCourseCount(searchTerm, null);
    }
    ...
    public Map<String, Integer> getCategoryCounts() throws SQLException {
        String sql = "SELECT cat.Name, COUNT(c.Id) as CourseCount " +
                "FROM Categories cat " +
                "LEFT JOIN Courses c ON cat.Id = c.LeafCategoryId " +
                "WHERE cat.ParentId IS NOT NULL " + // Only leaf categories
                "GROUP BY cat.Name";
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            while (rs.next()) {
                categoryCounts.put(rs.getString("Name"), rs.getInt("CourseCount"));
            }
        }
