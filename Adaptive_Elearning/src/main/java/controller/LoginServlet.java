...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (user != null) {
            ...
            if ("Inactive".equalsIgnoreCase(user.getRole())) {
                request.setAttribute("alertMessage", "Your account has been banned. Please contact administrator for more information.");
                request.setAttribute("alertStatus", false);
                request.setAttribute("showForm", "login");
            ...
            switch (role) {
                ...
                case "Instructor":
                    response.sendRedirect(request.getContextPath() + "/instructor_dashboard.jsp");
                    break;
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // Check if redirected from Google OAuth with banned account
        String bannedParam = request.getParameter("banned");
        if ("true".equals(bannedParam) && session != null) {
            String bannedEmail = (String) session.getAttribute("bannedEmail");
            String bannedMessage = (String) session.getAttribute("bannedAccountMessage");
            
            if (bannedEmail != null && bannedMessage != null) {
                request.setAttribute("alertMessage", bannedMessage);
                request.setAttribute("alertStatus", false);
                request.setAttribute("isBanned", true);
                request.setAttribute("bannedEmail", bannedEmail);
                
                // Clear session attributes
                session.removeAttribute("bannedEmail");
                session.removeAttribute("bannedAccountMessage");
            }
        }
