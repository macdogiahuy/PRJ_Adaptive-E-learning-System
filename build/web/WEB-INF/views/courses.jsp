...
<html>
    ...
    <body>
        ...
        <!-- Hero area: auto-rotating animated SVGs or intro video -->
                <div id="heroArea" style="width:100%;max-width:1100px;height:320px;overflow:hidden;border-radius:28px;margin:28px auto;box-shadow:0 6px 36px 0 rgba(31,38,135,0.10);position:relative;z-index:1;backdrop-filter: blur(4px);background:linear-gradient(90deg, rgba(255,255,255,0.02), rgba(0,0,0,0.02));">
                    <video id="heroVideo" style="width:100%;height:100%;object-fit:cover;display:none;" muted playsinline loop></video>
                    <div id="heroSlides" style="width:100%;height:100%;position:relative;">
                        <!-- animated hero SVGs will be injected here as iframes for isolation -->
                    </div>
                </div>
            ...
            <div class="row g-4">
                ...
                <script>
                    (function(){
                        ...
                        const curatedSvgs = [
                            ctxPath + '/assets/img/hero/hero1.svg',
                            ctxPath + '/assets/img/hero/hero2.svg',
                            ctxPath + '/assets/img/hero/hero3.svg'
                        ];

                        function mountSvgs(list){
                            ...
                            list.forEach((url, i)=>{
                                ...
                                ifr.style.display = i === 0 ? 'block' : 'none';
                                ifr.className = 'hero-slide-iframe';
                                slidesContainer.appendChild(ifr);
                            });
                        }
                        ...
                        function startAutoRotate(){
                            let idx = 0;
                            setInterval(()=>{
                                const slides = document.querySelectorAll('.hero-slide-iframe');
                        ...
                        // prefer hero video if available
                        fetch(ctxPath + '/assets/media/hero.mp4', { method: 'HEAD' }).then(resp => {
                            if(resp.ok){
                                heroVideo.src = ctxPath + '/assets/media/hero.mp4';
                                heroVideo.style.display = 'block';
                                heroVideo.play().catch(()=>{});
                            } else {
                                mountSvgs(curatedSvgs);
                                startAutoRotate();
                            }
                        }).catch(()=>{
                            mountSvgs(curatedSvgs);
