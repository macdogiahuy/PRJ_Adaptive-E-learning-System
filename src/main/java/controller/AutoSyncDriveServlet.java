...
public class AutoSyncDriveServlet extends HttpServlet {
    ...
    private static Map<String, Map<String, String>> sectionCache = new HashMap<>(); // courseId -> sectionName -> sectionId
    private static Map<String, Map<String, String>> lectureCache = new HashMap<>(); // sectionId -> lectureName -> lectureId
                        ...
                        for (File videoFile : videoFiles) {
                            ...
                            if (!isVideoExistsInDB(lectureId, fileId)) {
                                ...
                                try {
                                    ...
                                    syncLog.append(String.format("      ❌ Exception: %s - %s\n", fileName, e.getMessage()));
    ...
    private List<File> getSubfolders(Drive driveService, String parentId) throws IOException {
        String query = String.format("'%s' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false", parentId);
    ...
    private List<File> getVideoFiles(Drive driveService, String parentId) throws IOException {
        String query = String.format("'%s' in parents and (mimeType contains 'video/' or name contains '.mp4' or name contains '.avi' or name contains '.mov') and trashed=false", parentId);
        FileList result = driveService.files().list()
                .setQ(query)
    ...
    private Connection getDBConnection() throws SQLException {
        ...
        String dbPassword = System.getenv().getOrDefault("DB_PASSWORD", "123456789");
