...
public class UserDAO {
    ...
    private static final String CHECK_EXIST = "SELECT COUNT(*) FROM Users WHERE UserName = ? OR Email = ?";
    ...
    private static final String LOGIN_SQL = "SELECT Id, UserName, Role, InstructorId FROM Users WHERE (UserName=? OR Email=?) AND Password=?";
    ...
    public Users checkLogin(String nameOrEmail, String password) {
        // Defensive checks to avoid NPE and improve diagnostics
        if (nameOrEmail == null || nameOrEmail.isBlank()) {
            System.out.println("[UserDAO] checkLogin: missing username/email");
            return null;
        }
        if (password == null) {
            System.out.println("[UserDAO] checkLogin: null password provided for " + nameOrEmail);
            return null;
        }
        ...
        try (Connection con = DBConnection.getConnection(); PreparedStatement ps = con.prepareStatement(LOGIN_SQL)) {
            ...
            System.out
                    .println("[UserDAO] checkLogin: nameOrEmail=" + nameOrEmail + " hashedPassword=" + hashedPassword);
    ...
    private static final String FIND_BY_EMAIL = "SELECT TOP 1 Id, UserName, Email, Role, Token, LastModificationTime FROM Users WHERE Email = ?";
    ...
    private static final String FIND_BY_EMAIL_WITH_VALID_OTP = """
            ...
            """;
    private static final String ISSUE_RESET_OTP = "UPDATE Users SET Token = ?, LastModificationTime = GETDATE() WHERE Id = ?";

    private static final String UPDATE_PASSWORD_HASH = "UPDATE Users SET Password = ?, Token = '', LastModificationTime = GETDATE() WHERE Id = ?";

    private static final String CLEAR_TOKEN = "UPDATE Users SET Token = '' WHERE Id = ?";
    ...
    public void issueResetOtp(String userId, String otp6) {
        try (Connection con = DBConnection.getConnection();
                PreparedStatement ps = con.prepareStatement(ISSUE_RESET_OTP)) {
            ps.setString(1, otp6);
    ...
    public Users findByEmailWithValidOtp(String email, String otp6, int ttlMinutes) {
        try (Connection con = DBConnection.getConnection();
                PreparedStatement ps = con.prepareStatement(FIND_BY_EMAIL_WITH_VALID_OTP)) {
            ps.setString(1, email);
            ps.setString(2, otp6);
    ...
    public boolean updatePasswordHash(String userId, String newPlainPassword) {
        ...
        try (Connection con = DBConnection.getConnection();
                PreparedStatement ps = con.prepareStatement(UPDATE_PASSWORD_HASH)) {
    ...
    public Users findOrCreateGoogleUser(String email, String name, String avatarUrl, String googleId) {
        ...
        newUser.setPassword(""); // Google account
