...
<html>
  <head>
    ...
    <link
      rel="stylesheet"
      href="${pageContext.request.contextPath}/assets/css/course-player.css"
    />
      ...
      <div class="sidebar">
        ...
        <c:forEach var="section" items="${sidebarData}">
          ...
          <c:forEach var="lecture" items="${section.lecturesCollection}">
            <c:set
              var="activeClass"
              value="${lecture.id == currentLecture.id ? 'active' : ''}"
            />
            <a
              class="lecture-item ${activeClass}"
          ...
          <c:forEach var="assignment" items="${section.assignmentsCollection}">
            ...
            <a class="lecture-item" href="${assignmentUrl}">
              <span class="icon-checkbox ${completedClass}">
                <c:choose>
                  <c:when
                    test="${completedAssignments.contains(assignment.id)}"
                  >
                    &#10003;
                  </c:when>
        ...
        <div class="content-wrapper">
          ...
          <c:if test="${not empty currentMaterials}">
            ...
            <c:forEach var="material" items="${currentMaterials}">
              ...
              <c:set
                var="materialTypeLower"
                value="${fn:toLowerCase(material.type)}"
              />
              <c:set
                var="materialUrlLower"
                value="${fn:toLowerCase(material.url)}"
              />

              <c:choose>
                <%-- ✅ SỬA LỖI 3: DỌN DẸP ĐIỀU KIỆN DOCX --%>
                <c:when
                  test="${materialTypeLower eq 'doc' || materialTypeLower eq 'docx' || fn:endsWith(materialUrlLower, '.doc') || fn:endsWith(materialUrlLower, '.docx')}"
                >
                  <div class="material-doc" style="margin-bottom: 20px">
                    <h4>📄 Tài liệu</h4>
                    <div
                      id="docx-container-${material.lectureMaterialPK.id}"
                      class="docx-viewer"
                      style="
                        width: 100%;
                    ...
                    <script>
                      (function () {
                        ...
                        const container = document.getElementById(
                          'docx-container-${material.lectureMaterialPK.id}',
                        );
                        fetch(fileUrl)
                          .then((response) => response.blob())
                          .then((blob) => {
                            docx.renderAsync(blob, container).catch((err) => {
                              console.error('Lỗi render docx:', err);
                              container.innerHTML =
                                '<p style="color: red;">Lỗi: Không thể hiển thị file này.</p>';
                            });
                          })
                          .catch((err) => {
                            console.error('Lỗi fetch file docx:', err);
                            container.innerHTML =
                              '<p style="color: red;">Lỗi: Không thể tải file từ server.</p>';
                          });
                      })();
                    </script>
                  </div>
                ...
                <c:when
                  test="${materialTypeLower eq 'pdf' || fn:endsWith(materialUrlLower, '.pdf')}"
                >
                ...
                <c:when
                  test="${materialTypeLower eq 'video' || fn:endsWith(materialUrlLower, '.mp4') || fn:endsWith(materialUrlLower, '.webm') }"
                >
                  <c:set
                    var="videoUrl"
                    value="${fn:startsWith(material.url, 'http') ? material.url : streamUrl}"
                  />
